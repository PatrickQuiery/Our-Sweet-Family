 ...
 */
export function GlassView({ children, style, intensity = 44, radius }: GlassProps) {
  const { scheme, radius: r } = useTheme();
  const isDark = scheme === 'dark';
  const br = radius ?? r.lg;
  return (
    <View style={[{ borderRadius: br, overflow: 'hidden', borderWidth: 1, borderColor: isDark ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.65)' }, style]}>
      <BlurView intensity={intensity} tint={isDark ? 'dark' : 'light'} style={StyleSheet.absoluteFill} />
      <View style={[StyleSheet.absoluteFill, { backgroundColor: isDark ? 'rgba(30,33,54,0.32)' : 'rgba(255,255,255,0.34)' }]} />
      {/* top specular sheen */}
